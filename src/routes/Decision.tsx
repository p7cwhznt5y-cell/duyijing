...
export function Decision(_props: RoutableProps) {
  ...
  const [knowsHexagrams, setKnowsHexagrams] = useState<boolean | null>(null);
  ...
  if (category) {
    ...
    } else if (matches.length > 1 && !autoGenerated) {
  ...
  return arr;
}, [category, selectedHexagram, autoGenerated]);
  ...
  const handleSelectCategory = (cat: HexagramCategory) => {
    setCategory(cat);
    setSelectedHexagram(null); // 不再自动选择，等待用户选择起卦方式
    setYaoChoices({});
    setAiInterpretation("");
    setError("");
    setReportRequested(false);
    setKnowsHexagrams(null);
  ...
  const autoHexagram = matches[index];
  
  // 先设置卦象和自动生成标记
  setSelectedHexagram(autoHexagram);
  setAutoGenerated(true);
  setKnowsHexagrams(false);
  
  // 使用 setTimeout 确保 steps 更新后再跳转
  setTimeout(() => {
    setStep(2);
  }, 0);
};
...
setAutoGenerated(false);
  ...
  const canNext = (): boolean => {
    switch (currentStep.type) {
      ...
      case "category":
        return category !== null && knowsHexagrams === true;
  ...
  const handlePrev = () => {
  if (currentStep.type === "hexagram-select") {
    setKnowsHexagrams(null);
  }
    ...
    <main class="max-w-md md:max-w-2xl mx-auto px-4 py-6 font-sans bg-paper">
      ...
      {currentStep.type === "category" ? (
        <section>
          ...
          />
          {currentStep.type === "category" && category && !autoGenerated && (
  <div class="mt-6 p-4 border-t border-ink/10">
    <p class="font-sans text-sm text-ink/70 mb-3">请问您熟悉64卦的含义吗？</p>
    <div class="flex gap-3">
      <button
        onClick={handleAutoGenerate}
        class="flex-1 py-2 border border-ink/20 rounded-lg text-sm text-ink hover:bg-ink/5"
      >
        不太了解，让系统结合当下推荐
      </button>
      <button
        onClick={() => setKnowsHexagrams(true)}
        class="flex-1 py-2 bg-cinnabar text-white rounded-lg text-sm font-medium"
      >
        我了解，我自己选
      </button>
    </div>
